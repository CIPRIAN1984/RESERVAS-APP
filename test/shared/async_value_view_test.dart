...
void main() {
  ...
  testWidgets('muestra el estado vacío cuando la lista está vacía', (tester) async {
    await tester.pumpWidget(_host(const AsyncData<List<String>>([])));
    expect(find.text('Nada por aquí.'), findsOneWidget);
  });

  testWidgets('muestra el error con reintento cuando no hay datos previos', (tester) async {
    await tester.pumpWidget(_host(AsyncError(Exception('boom'), StackTrace.empty)));
