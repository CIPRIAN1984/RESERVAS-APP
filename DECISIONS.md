...
## 2026-09-14 — Una puerta de solo lectura para el agente personal del Dueño

Cipri quiere que su asistente (ChatGPT) pueda preguntarle cosas a la app:
cuánta gente vino, quién no ha pagado, quién lleva semanas sin aparecer, qué
clases hay esta semana. Lo que se ha montado y por qué:

**Cómo entra.** Una Edge Function (`agente`) con `verify_jwt = false`, porque
un agente externo no tiene sesión de Supabase. Se autentica con una clave
`itc_…` que el Dueño genera desde *Herramientas → Claves para tu agente*. La
clave se guarda **troceada con sha256**: se enseña entera una sola vez, al
crearla, y después no hay forma de recuperarla — si se pierde, se anula y se
hace otra.

**Habla MCP.** La misma URL atiende JSON-RPC 2.0 (`initialize`, `tools/list`,
`tools/call`), que es lo que entienden ChatGPT y Claude como «conector», y
además un `GET ?consulta=…` para probar con curl o enganchar cualquier otro
programa.

**Solo lee, por construcción.** No hay ni una ruta de escritura. Las cuatro
consultas (`agente_resumen`, `agente_avisos`, `agente_horario`,
`agente_alumnos`) son funciones `stable` y el único `insert` que hace la
función es el de su propio registro de accesos.

**Las consultas NO son `security definer`.** Podrían serlo, tomando la
academia del parámetro, pero entonces un fallo futuro que les concediera
`execute` a `authenticated` dejaría a cualquier alumno leer la academia
entera pasando otro uuid. Al dejarlas como invocador, quien las llama es
`service_role` y basta con eso; si alguien se equivoca con los permisos, RLS
sigue tapando el agujero. La única excepción es `agente_correo_de`, y por un
motivo concreto: **`service_role` no tiene permiso de lectura sobre
`auth.users` en producción**. Se queda lo más estrecha posible — un correo,
de un alumno, de la academia que se le pasa.

**El correo va detrás de su propio interruptor** (`incluye_contacto`, apagado
por defecto). Se le advirtió a Cipri que ese dato sale de la app hacia la
empresa del agente y aun así lo quiso; tenerlo aparte permite cerrarlo sin
romper el resto. El interruptor lo manda **la clave**, no el argumento de la
llamada: si viniera del argumento, bastaría con pedirlo para saltárselo.

**Lo que NO existe:** teléfonos. La app no guarda ningún teléfono de alumno —
el campo `telefono` es de `academias`, o sea el de la propia academia. El
único dato de contacto de un alumno es su correo de registro, y solo lo
tienen quienes tienen cuenta propia (los hijos dados de alta por un padre,
no).

**Queda fuera:** ingresos y previsión de cobros. `tarifas.periodicidad` tiene
varios valores y sumar euros a ojo daría una cifra que parece buena y no lo
es. Cuando haga falta, se hace aparte y con su prueba.
