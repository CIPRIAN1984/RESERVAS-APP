...
class _TarifasAlumnoViewState extends ConsumerState<_TarifasAlumnoView> {
  ...
  Future<void> _suscribirse(Tarifa tarifa) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(mensajeErrorAmigable(e, generico: 'No se ha podido iniciar la suscripción.'))),
        );
      }
    } finally {
  ...
  Future<void> _cancelar(String suscripcionId) async {
    ...
    try {
      await ref.read(tarifasRepositoryProvider).cancelarSuscripcion(suscripcionId);
      ref.invalidate(suscripcionActivaProvider(widget.alumnoId));
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('No se ha podido cancelar la suscripción.')));
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    final suscripcionAsync = ref.watch(suscripcionActivaProvider(widget.alumnoId));
    final tarifasAsync = ref.watch(tarifasProvider(true));

    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        Text('Tu tarifa actual', style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 8),
        suscripcionAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, st) => Text('No se ha podido cargar tu suscripción.',
            ...
            return Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(suscripcion.tarifaNombre ?? 'Tarifa', style: Theme.of(context).textTheme.titleMedium),
                    ...
                    if (enProceso) ...[
                      ...
                      Align(
                        ...
                        child: OutlinedButton(
                          onPressed: _cancelando ? null : () => _cancelar(suscripcion.id),
                          child: _cancelando
                              ? const SizedBox(
                                  width: 16,
                                  height: 16,
        ...
        Text('Tarifas disponibles', style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 8),
        tarifasAsync.when(
          loading: () => const Center(child: CircularProgressIndicator()),
          error: (e, st) => Text('No se han podido cargar las tarifas.',
              style: TextStyle(color: Theme.of(context).colorScheme.error)),
          data: (tarifas) {
...
class _TarifasGestionView extends ConsumerWidget {
  ...
  Future<void> _alternarActivo(WidgetRef ref, Tarifa tarifa) async {
    await ref.read(tarifasRepositoryProvider).alternarActivo(tarifa.id, !tarifa.activo);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          await Navigator.of(context).push(
            MaterialPageRoute(builder: (_) => CrearTarifaScreen(academiaId: academiaId)),
          );
          ref.invalidate(tarifasProvider(false));
        },
      ...
      body: tarifasAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (e, st) => Center(
          child: Text('No se han podido cargar las tarifas.',
              style: TextStyle(color: Theme.of(context).colorScheme.error)),
        ),
        data: (tarifas) {
          if (tarifas.isEmpty) {
