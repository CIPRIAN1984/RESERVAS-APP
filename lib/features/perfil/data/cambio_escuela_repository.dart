...
class CambioEscuelaRepository {
  ...
  Future<List<MiSolicitudCambio>> misSolicitudes() async {
    ...
    return rows
        .map((r) => MiSolicitudCambio.fromRow(r as Map<String, dynamic>))
        .toList();
  }

  Future<List<SolicitudPendiente>> listarPendientes() async {
    final rows =
        await _client.rpc('listar_solicitudes_pendientes_destino') as List;
    return rows
        .map((r) => SolicitudPendiente.fromRow(r as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<void> resolver({
    ...
    await _client.rpc(
      ...
      params: {'p_solicitud_id': solicitudId, 'p_aprobar': aprobar},
    );
  }
}
